...
function generateLayersFromData (data) {
  ...
  for (let i = 0; i < data.length; i++) {
    const idx = layers.findIndex(layer => layer.name === data[i].TypeCode)
    if (idx === -1) {
...
export default function Map (props) {
  ...
  if (data === null) {
    ...
    const layers = generateLayersFromData(data)
        ...
        <MapContainer style={{ height: '95vh', overflow: 'hidden' }} center={[35.227085, -80.843124]} zoom={11}>
          ...
          <LayersControl position='topright'>
            {layers.map(layer => {
              ...
              return (
                <LayersControl.Overlay checked name={layerControlText} key={layer.name}>
                  <LayerGroup>
